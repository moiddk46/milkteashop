<template>
  <div class="m-auto border border-light rounded bg-warning rounded w-50">
    <div class="d-flex justify-content-center">
      <form @submit.prevent="edit" class="w-50">
        <h2 class="font-weight-bold text-center fw-bold fs-2 my-2">Đăng ký</h2>
        <div class="mb-3">
        ...
        <div class="mb-3">
          ...
          <input
            ...
            v-model="khachhang.pass1"
            required
          />
        </div>
        <span
          v-if="error.pass1 == false"
          class="text-danger"
          style="font-size: 13px"
        >
          <i class="fa-solid fa-triangle-exclamation"></i> Mật khẩu không trùng
          khớp!
        </span>
        <div class="d-flex justify-content-center">
...
export default {
  ...
  methods: {
    async edit() {
      ...
      if (this.khachhang.pass1.value == this.khachhang.pass.value) {
        this.error.pass1 = true;
      } else {
        this.error.pass1 == false;
      }
      if (
        this.error.name == true &&
        this.error.diachi == true &&
        ...
        try {
            await detailkh.add(this.khachhang).then(
